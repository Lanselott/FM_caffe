...
class BaseConvolutionLayer : public Layer<Dtype> {
 ...
 void forward_cpu_gemm_xu_xuv(const Dtype* input,Dtype* input2, const Dtype* weights,const Dtype* weights2,
  ...
  void forward_cpu_xu_bias(Dtype* output, const Dtype* bias);
...
#ifndef CPU_ONLY
  void forward_gpu_gemm_xu_xuv(const Dtype* input,Dtype* input_xu,
  const Dtype* weights,const Dtype* weights2, Dtype* output, Dtype* output2,int batch_idx, const vector<Blob<Dtype>*>& bottom,
  bool skip_im2col = false);
  bool forward_gpu_gemm_ccnmm_merge(const Dtype* input,
  ...
  Blob<Dtype> xu_bias_multiplier_;
...
protected:
  ...
  inline void conv_im2col_gpu_xu(const Dtype* data, Dtype* col_buff) {
    if (!force_nd_im2col_ && num_spatial_axes_ == 2) {
      im2col_gpu(data, conv_in_channels_,
          conv_input_shape_.cpu_data()[1], conv_input_shape_.cpu_data()[2],
          kernel_shape_.cpu_data()[0], kernel_shape_.cpu_data()[1],
