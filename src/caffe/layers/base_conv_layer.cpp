...
step = this->layer_param_.sparse_param().step();
  ...
  if(step == 0){
    ...
    if(bottom.size() != 4) LOG(FATAL)<<"Sparse layer step 4 should be have x,s,v,lr_bias 4 bottoms";
  //same as step = 1
...
if(step == 3)    {
  ...
  row_buf_mask_ccnmm.Reshape(1,1,1,conv_out_channels_);//now sparse is kernel, so just use conv_out_channels_
}
  ...
  if (bias_term_ && step!=3) {
    vector<int> bias_multiplier_shape(1, out_spatial_dim_);
    bias_multiplier_.Reshape(bias_multiplier_shape);
    caffe_set(bias_multiplier_.count(), Dtype(1),
        bias_multiplier_.mutable_cpu_data());
  }
  else if(bias_term_ && step == 3) { // two biases: xu's bias and xs's bias
    vector<int> bias_multiplier_shape(1, out_spatial_dim_); //xs's
    vector<int> xu_bias_multiplier_shape(1,xu_buffer_.count(2));

    bias_multiplier_.Reshape(bias_multiplier_shape);
    xu_bias_multiplier_.Reshape(xu_bias_multiplier_shape);
    caffe_set(bias_multiplier_.count(), Dtype(1),
        bias_multiplier_.mutable_cpu_data());
    caffe_set(xu_bias_multiplier_.count(), Dtype(1),
        xu_bias_multiplier_.mutable_cpu_data());
  }
}
...
void BaseConvolutionLayer<float>::forward_cpu_gemm_xu_xuv(const float* input,float* input_xu,
  ...
  const float* bias = bottom[3]->cpu_data(); //xu's bias_shape
  //LOG(INFO)<<"xu's bias shape:"<<bottom[3]->shape(0)<<","<<bottom[3]->shape(1)<<","<<bottom[3]->shape(2)<<","<<bottom[3]->shape(3);
  float *input_padded;
...
//xu setting
  if (!is_1x1_ ||  is_concatenating_weights_features_) {
   ...
   if (!skip_im2col || is_concatenating_weights_features_) {
     //LOG(INFO)<<"check is_concatenating_weights_features_";
  ...
  for (int g = 0; g < group_; ++g) {
    ...
    if(this->bias_term_){
    //  LOG(INFO)<<"xu bias";
      caffe_cpu_gemm<float>(CblasNoTrans, CblasNoTrans, xu_buffer_.shape(0),
            xu_buffer_.count(2), 1, (float)1., bias, xu_bias_multiplier_.cpu_data(),
            (float)1., output);
    }
  }
...
//LOG(INFO)<<"www"<<col_buf_mask_ccnmm.shape(0)<<","<<col_buf_mask_ccnmm.shape(1)<<","<<col_buf_mask_ccnmm.shape(2)<<","<<col_buf_mask_ccnmm.shape(3);
...
template <typename Dtype>
void BaseConvolutionLayer<Dtype>::forward_cpu_xu_bias(Dtype* output,
    const Dtype* bias) {
  caffe_cpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, xu_buffer_.shape(0),
      xu_buffer_.count(2), 1, (Dtype)1., bias, xu_bias_multiplier_.cpu_data(),
      (Dtype)1., output);
}
...
//xu,xuv operator
template <typename Dtype>
void BaseConvolutionLayer<Dtype>::forward_gpu_gemm_xu_xuv(const Dtype* input,Dtype* input_xu,
    const Dtype* weights,const Dtype* weights2, Dtype* output, Dtype* output2,int batch_idx, const vector<Blob<Dtype>*>& bottom,
    bool skip_im2col) {

      const Dtype* col_buff = input;
      const Dtype* bias = bottom[3]->gpu_data(); //xu's bias_shape
      //LOG(INFO)<<"xu's bias shape:"<<bottom[3]->shape(0)<<","<<bottom[3]->shape(1)<<","<<bottom[3]->shape(2)<<","<<bottom[3]->shape(3);
      Dtype *input_padded;
      int input_padded_len;
    //xu setting
      if (!is_1x1_) {
       //if (tid == 0) im2col_time -= omp_get_wtime();
       if (!skip_im2col) {
           conv_im2col_gpu_xu(input, col_buffer_xu.mutable_gpu_data());
       }
       col_buff = col_buffer_xu.gpu_data();
       //LOG(INFO)<<"xu :col_buffer_ size: "<<col_buffer_xu.shape(0)<<","<<col_buffer_xu.shape(1)<<","<<col_buffer_xu.shape(2);
      ...
      for (int g = 0; g < group_; ++g) {
        ...
        //if(this->bias_term_){
        //  LOG(INFO)<<"xu bias";
          caffe_gpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, xu_buffer_.shape(0),
                xu_buffer_.count(2), 1, (Dtype)1., bias, xu_bias_multiplier_.gpu_data(),
                (Dtype)1., output);
        //}
      }
      Dtype* col_buff_xuv = input_xu;
    /*
      //xuv setting
        if (!is_1x1_ ||  is_concatenating_weights_features_) {
        //  if (tid == 0) im2col_time -= omp_get_wtime();
          ...
          if (!skip_im2col || is_concatenating_weights_features_) {
            offset = bottom[2]->count(1) * xuv_buffer_.count(2)*group_*batch_idx;
            Dtype *col_buff_mutable = col_buffer_xuv.mutable_gpu_data() + offset;
          	  conv_im2col_gpu_xuv_nc(input_xu, col_buff_mutable);

          }
          col_buff_xuv = col_buffer_xuv.mutable_gpu_data() + offset;
        //  LOG(INFO)<<"xuv col_buffer_ size: "<<col_buffer_xuv.shape(0)<<","<<col_buffer_xuv.shape(1)<<","<<col_buffer_xuv.shape(2);
          if (tid == 0) im2col_time += omp_get_wtime();
        }
    */
        ...
        for (int g = 0; g < group_; ++g) {
          ...
          caffe_gpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, M_, N_, K_,
      				  (Dtype)1., weights2 , col_buff_xuv,
      				  (Dtype)0., output2);
      		//LOG(INFO)<<"caffe_gpu_gemm time: "<<omp_get_wtime() - k;
...
template<typename Dtype>
bool BaseConvolutionLayer<Dtype>::forward_gpu_gemm_ccnmm_merge(const Dtype* input,
    const Dtype* weights, Dtype* output, int batch_idx, const vector<Blob<Dtype>*>& bottom,
    bool skip_im2col)
{
  const Dtype* col_buff = input;
  //initial STEP
  ...
  //analyze column sparsity
    for (int g = 0; g < group_; ++g) {
      caffe_cpu_if_all_zero(this->blobs_[0]->shape(0)/group_,
          ...
          col_buf_mask_ccnmm.mutable_cpu_data() + this->blobs_[0]->count(1,4) * g);
    }
    //analyze row sparsity
    caffe_cpu_if_all_zero(this->blobs_[0]->shape(0),
                this->blobs_[0]->count(1,4),
                this->blobs_[0]->cpu_data(),
                row_buf_mask_ccnmm.mutable_cpu_data(),
                false);
...
// concatenating weight matrix
    ...
    int total_weights = 0;
			for (int g = 0; g < group_; ++g) {
				left_cols = kernel_dim_ - caffe_cpu_asum( kernel_dim_, col_buf_mask_ccnmm.cpu_data()+ kernel_dim_ * g);
				left_columns_.push_back(left_cols);
				left_rows = conv_out_channels_ /group_ - caffe_cpu_asum( conv_out_channels_ /group_, row_buf_mask_ccnmm.cpu_data()+ conv_out_channels_ /group_ * g);
        ...
        for (int g = 0; g < group_; ++g) {
          LOG(INFO)<<layerparam.name()<<" squeezing to "<<left_rows_[g]<<"x"<<left_columns_[g];
  				caffe_cpu_concatenate_rows_cols(
  						...
  						col_buf_mask_ccnmm.cpu_data()+ kernel_dim_ * g,
  						row_buf_mask_ccnmm.cpu_data()+ conv_out_channels_ /group_ * g
  						);
  				weight_offset_sum += left_rows_[g]*left_columns_[g];
        }
      }//end if
      else{check_zero = false;}
LOG(INFO)<<"weight matrix squeezed";

      initial_check = 0;
    }
//LOG(INFO)<<"check_zero:"<<check_zero;
    Blob<Dtype> input_buf;
	  ...
	  conv_im2col_cpu(input_buf.cpu_data(), col_buffer_.mutable_cpu_data(), col_buf_mask_ccnmm.mutable_cpu_data());
    ...
    */
    ...
    if(check_zero == true)
    {
      ...
      //  LOG(INFO)<<"left_cols && left_rows:"<<left_cols<<" "<<left_rows;
         ...
         col_buf_offset_sum += left_cols * conv_out_spatial_dim_;
  			  ...
  			  if(group_-1 == g){
  				  ...
  				  caffe_cpu_dispatch_rows(conv_out_channels_,conv_out_spatial_dim_,output_buf.mutable_cpu_data(),row_buf_mask_ccnmm.cpu_data());
...
/*
        caffe_cpu_cblas_gemm(xs_buffer_.shape(1) /
  				  group_, xs_buffer_.count(2), left_cols,
  				  (Dtype)1., squeezed_weight_buffer_ccnmm.cpu_data() + weight_offset_ * g,
  				  this->blobs_[0]->count(1) , col_buff + offset_sum,
  				  xs_buffer_.count(2), (Dtype)0., output + output_offset_ * g, xs_buffer_.count(2));
  		  offset_sum += left_cols * xs_buffer_.count(2);
        caffe_gpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans,
        						  left_rows, conv_out_spatial_dim_, left_cols,
        						 (Dtype)1., squeezed_weight_buffer_.gpu_data() + weight_offset_sum,
...
//    check_zero = false;
//  }
//LOG(INFO)<<layerparam.name()<<"check_zero:"<<check_zero;
  return check_zero;
}
